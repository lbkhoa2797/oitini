...
from config import CHUNKS_PATH, CHROMA_DIR, BM25_PATH, EMBED_MODEL, EMBED_BATCH_SIZE, DEVICE
...
def build_indices_gpu() -> None:
    ...
    # BGE-M3 defaults to an 8192-token context; attention memory grows with
    # seq_len^2, so a long chunk in a batch can spike VRAM into OOM.

    # Chunks are ~450 words so using max_sq_length = 1024 should be enough
    ...
    for i in tqdm(range(0, len(chunks), EMBED_BATCH_SIZE), desc="embed"):
